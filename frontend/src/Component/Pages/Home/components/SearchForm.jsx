...
const SearchForm = ({ 
    ...
    const handleFormSubmit = (e) => {
        ...
        if (validUrls.length === 0) {
            alert(language === "ru" ? "Пожалуйста, введите хотя бы один URL" : "Please enter at least one URL");
            return;
        }
        // Передаём все URL'ы
        setQuery(validUrls);
    ...
    return (
        <form className="form__search" onSubmit={handleFormSubmit}>
            ...
            <div className="urls-container" style={{ marginBottom: '12px' }}>
                {urls.map((url, index) => (
                    <div key={index} className="form__row" style={{ marginBottom: '8px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <input
                            type="text"
                            value={url}
                            onChange={(e) => handleUrlChange(index, e.target.value)}
                            placeholder={language === "ru" ? "Адрес сайта (пример: https://example.com)" : "Website URL (example: https://example.com)"}
                            aria-label={language === "ru" ? `URL ${index + 1}` : `URL ${index + 1}`}
                            disabled={loading}
                            style={{ flex: 1 }}
                        />
                        {urls.length > 1 && (
